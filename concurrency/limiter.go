...
// Limiter defines the interface for a concurrency control mechanism.
// A Limiter ensures that a limited number of concurrent operations (or "slots") can
...
type Limiter interface {
	...
	// Acquire provides access to the internal slots of the limiter.
	...
	// by other tasks. If the limiter is already closed or if the slots buffer is full,
	// the release is safely ignored.
	Release()

	// Close shuts down the limiter and releases all resources.
	// Once closed:
